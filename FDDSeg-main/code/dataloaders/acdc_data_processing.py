...
slice_num = 0
mask_path = sorted(
    glob.glob("../data/MSCMR_training/*_gt.nii.gz"))
for case in mask_path:
    ...
    for slice_ind in range(image.shape[0]):
        f = h5py.File(
            '../data/MSCMR/MSCMR_training_slices/{}_slice_{}.h5'.format(item, slice_ind), 'w')
        f.create_dataset(
...
for case in mask_path:
    ...
    print(item)
    f = h5py.File(
        '../data/MSCMR/MSCMR_training_volumes/{}.h5'.format(item), 'w')
    f.create_dataset(
