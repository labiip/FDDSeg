...
class BaseDataSets(Dataset):
    def __init__(self, base_dir=None, split='train', transform=None, fold="fold1", sup_type="label"):
        ...
        train_ids, test_ids = self._get_fold_idsacdcc(fold)  # here changes!!!
        if self.split == 'train':
            self.all_slices = os.listdir(
                self._base_dir + "/ACDCC_training_slices")
            self.sample_list = []
            ...
            self.all_volumes = os.listdir(
                self._base_dir + "/ACDCC_training_volumes")
            self.sample_list = []
            for ids in test_ids:
                new_data_list = list(filter(lambda x: re.match(
    ...
    def _get_fold_ids(self, fold):
        ...
        fold1_testing_set = [
            "patient{:0>3}".format(i) for i in range(41, 46)]
        if fold == "fold1":
            return [fold1_training_set, fold1_testing_set]
        else:
            return "ERROR KEY"

    def _get_fold_idsacdcc(self, fold):
        all_cases_set = ["patient{:0>3}".format(i) for i in range(1, 41)]
        fold1_training_set = [
            "patient{:0>3}".format(i) for i in range(1, 95)]
        fold1_testing_set = [
            "patient{:0>3}".format(i) for i in range(131, 161)]
        if fold == "fold1":
            return [fold1_training_set, fold1_testing_set]
        else:
            return "ERROR KEY"
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            h5f = h5py.File(self._base_dir +
                            "/ACDCC_training_slices/{}".format(case), 'r')
        else:
            h5f = h5py.File(self._base_dir +
