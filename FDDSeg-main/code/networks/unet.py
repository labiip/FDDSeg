...
class Decoderr(nn.Module):
    def __init__(self, params):
        ...
        self.out_conv = nn.Conv2d(self.ft_chns[0], self.n_class,
                                  kernel_size=3, padding=1)
        self.dow = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, padding=1),
            nn.Conv2d(in_channels=32, out_channels=self.n_class, kernel_size=3, padding=1)
        )

        self.dow64_dwt = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, padding=1),
            # nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=16, kernel_size=3, padding=1),
            # nn.ReLU(),
            nn.Conv2d(in_channels=16, out_channels=1, kernel_size=3, padding=1),
            # nn.Conv2d(in_channels=self.n_class, out_channels=1, kernel_size=3, padding=1)
        )

        self.dow32 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, padding=1),
            nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, padding=1),
            nn.Conv2d(in_channels=32, out_channels=self.n_class, kernel_size=3, padding=1)
        )
        self.dow128 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=self.n_class, kernel_size=3, padding=1)
        )
        self.trans = nn.Sequential(
            nn.Conv2d(in_channels=self.n_class, out_channels=self.n_class, kernel_size=3, padding=1),
            nn.Conv2d(in_channels=self.n_class, out_channels=self.n_class, kernel_size=1, padding=0)
        )
    ...
    def forward(self, feature):
        ...
        x_32 = self.dow32(x_32)
        x = self.up2(x, x2)
        x_64 = x
        x_64_org = x_64
        x_64_dwt = self.dow64_dwt(x_64)
        x_64 = self.dow(x_64)
        x = self.up3(x, x1)
        x_128 = x
        x_128_org = x_128

        x_128 = self.dow128(x_128)
        x = self.up4(x, x0)
        x_256_org = x
        output = self.out_conv(x)

        return output, x_32, x_64, x_128, x_32_org, x_64_org, x_128_org, x_64_dwt
...
class UNet_CCTT(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_CCTT, self).__init__()
...
class UNet_CCT1(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        params = {'in_chns': in_chns,
                  ...
                  'acti_func': 'relu'}
        self.encoder = Encoder(params)
        self.main_decoder = Decoderr(params)

    def forward(self, x):
        # original shape
        x0 = x
        feature1 = self.encoder(x)
        main_seg, x_32, x_64, x_128, x_32_org, x_64_org, x_128_org, x_64_dwt = self.main_decoder(feature1)
        return main_seg, x_32, x_64, x_128

class UNet_CCT2(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_CCT2, self).__init__()
        ...
        self.aux_decoder1 = Decoderr(params)

    def forward(self, x):
        # original shape
        x0 = x
        feature2 = self.encoder2(x)
        aux1_feature = [Dropout(i) for i in feature2]
        aux_seg1, aux_x_32, aux_x_64, aux_x_128, aux_x_32_org, aux_x_64_org, aux_x_128_org, x_64_dwt_aux = self.aux_decoder1(aux1_feature)
        return aux_seg1, aux_x_32, aux_x_64, aux_x_128
